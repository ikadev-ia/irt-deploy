...
$batch_age = 0;
$needs = ['daily_feed_kg' => 0];
if ($batch) {
    $start_date = new DateTime($batch['start_date']);
    $today = new DateTime();
    $batch_age = $start_date->diff($today)->days;
    $needs = calculateChickenNeeds($batch_age, $batch['current_birds'], $weather);
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-item.active { background: var(--white); color: var(--green); box-shadow: 0 6px 14px rgba(0,0,0,0.15); }
        ...
        .settings-sub a:hover { background: var(--yellow); color: #14B53A; }
        ...
        .form-header p { font-size: 0.8rem; opacity: 0.8; }
        .home-btn {
            position: absolute; top: 18px; right: 20px; background: rgba(30,41,59,0.12);
            color: #1e293b; padding: 5px 12px; border-radius: 30px; text-decoration: none;
            font-size: 0.7rem; font-weight: 600; display: inline-flex; align-items: center; gap: 5px;
            transition: 0.2s;
        }
        .home-btn i { font-size: 0.7rem; margin: 0; }
        .home-btn:hover { background: rgba(30,41,59,0.25); }
        .form-body { padding: 30px; }
        .batch-info {
            background: rgba(20,181,58,0.1); border-radius: 28px; padding: 12px 18px;
            margin-bottom: 25px; display: flex; justify-content: space-between;
            align-items: center; flex-wrap: wrap; gap: 10px; font-size: 0.85rem;
            border: 1px solid var(--border-light);
        }
        .batch-badge { background: var(--yellow); color: #1e293b; padding: 4px 12px; border-radius: 30px; font-weight: 700; font-size: 0.75rem; margin-left: 8px; }
        ...
        .btn-submit {
            ...
            border: none; padding: 14px; border-radius: 40px; font-size: 1rem; font-weight: 700;
            cursor: pointer; transition: 0.2s; display: flex; align-items: center;
            justify-content: center; gap: 8px; margin-top: 15px;
        }
        .btn-submit:hover { background: var(--green-dark); transform: translateY(-1px); box-shadow: 0 8px 20px rgba(20,181,58,0.3); }
        .toast {
            position: fixed; bottom: 30px; right: 30px; background: var(--green); color: white;
            padding: 10px 20px; border-radius: 40px; font-weight: 500; z-index: 2000;
...
<div class="main-content">
    <div class="form-container">
        <div class="form-header">
            ...
            <h1>Enregistrement quotidien</h1>
            <b><p>Relevés du jour</p></b>
        ...
        <div class="form-body">
            <?php if ($batch): ?>
                <div class="batch-info">
                    <div>
                        <i class="fas fa-chicken"></i> <strong><?php echo htmlspecialchars($batch['name']); ?></strong>
                ...
                <form id="dailyForm">
                    ...
                    <!-- Champs cachés pour satisfaire add_daily_data.php -->
                    <input type="hidden" name="humidity" value="<?php echo $weather['humidity']; ?>">
                    ...
                    <div class="double-row">
                        <div class="input-group">
                            <label><i class=""></i> Morts aujourd'hui</label>
                            <input type="number" name="mortality" value="0" min="0">
                        </div>
                        <div class="input-group">
                            <label><i class=""></i> Malades / Anormaux</label>
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>Créez d'abord un lot pour enregistrer des données.</p>
                    <a href="add_lot.php" class="btn-primary"><i class="fas fa-plus-circle"></i> Créer un lot</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
<script>
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('add_daily_data.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (result.success) {
                    ...
                    toast.innerHTML = '<i class="fas fa-check-circle"></i> Données enregistrées !';
                    document.body.appendChild(toast);
                    setTimeout(() => toast.remove(), 2500);
                    document.querySelector('input[name="mortality"]').value = 0;
                    document.querySelector('input[name="sick"]').value = 0;
                } else {
